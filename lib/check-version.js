  ...
  axios({
    ...
    if(res.status === 200) {
      ...
      if(semver.lt(local, latest)) {
        log.tip();
        log.tip(chalk.blue('  A newer version of chare cli is available.'));
        log.tip();
        log.tip(`  latest:    ${chalk.green(latest)}`);
        log.tip(`  installed:    ${chalk.red(local)}`)
        log.tip(`  update chare latest: npm update -g chare`);
        log.tip();
      }
      done();
    }
    ...
    if(err) {
      ...
      if(res) {
        log.tip(chalk.red(`     ${res.statusText}: ${res.headers.status}`));
      } else {
        log.tip(chalk.red(`     ${err.message}`));
      }
      log.tip();
      done();
    }
